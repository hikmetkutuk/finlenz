      ...
      <div className="grid grid-cols-3 p-6 border-b border-slate-700">
        <div>
          ...
          <div className="text-xs text-slate-400 mt-1">
            {s1.companyName.toUpperCase()}
          </div>
          <div className="text-xl font-semibold mt-2">
            {s1.currentPrice.toLocaleString("tr-TR", {
              minimumFractionDigits: 2,
            })}{" "}
            ₺
          </div>
          <div
            className={`text-sm mt-1 ${s1.percentChange >= 0 ? "text-green-400" : "text-red-400"}`}
          >
            {s1.percentChange >= 0 ? "+" : ""}
            {s1.percentChange.toFixed(2)}%
          </div>
        </div>
        ...
        <div className="text-right">
          ...
          <div className="text-xs text-slate-400 mt-1">
            {s2.companyName.toUpperCase()}
          </div>
          <div className="text-xl font-semibold mt-2">
            {s2.currentPrice.toLocaleString("tr-TR", {
              minimumFractionDigits: 2,
            })}{" "}
            ₺
          </div>
          <div
            className={`text-sm mt-1 ${s2.percentChange >= 0 ? "text-green-400" : "text-red-400"}`}
          >
            {s2.percentChange >= 0 ? "+" : ""}
            {s2.percentChange.toFixed(2)}%
          </div>
        </div>
      </div>
        ...
        <div key={section.title}>
          ...
          {section.rows.map((row) => {
            ...
            return (
              <div
                key={row.key}
                className="grid grid-cols-3 px-6 py-3 border-t border-slate-800 hover:bg-slate-800/30 transition-colors"
              >
                <div className="text-sm text-slate-300">{row.label}</div>
                <div
                  className={`text-sm text-center font-medium ${b1 ? "text-green-400" : "text-slate-200"}`}
                >
                  {row.format(v1, s1.currency)}
                </div>
                <div
                  className={`text-sm text-right font-medium ${b2 ? "text-green-400" : "text-slate-200"}`}
                >
